...
export const GET: APIRoute = async ({ request }) => {
  try {
    ...
    if (!secureToken || !secureToken.includes('.')) {
      return new Response(JSON.stringify({ error: 'Acceso denegado. Token faltante o inválido.' }), {
    ...
    if (!secret) {
      ...
      return new Response(JSON.stringify({ error: 'Error del servidor. Configuración incompleta.' }), {
